  ...
  try {
    ...
    if (user.subscription === "free") {
      return NextResponse.json(
        { error: "L'export est réservé aux plans Pro et Premium" },
        { status: 403 }
      );
    }
    ...
    if (type === "payments") {
      ...
      csv =
        "Prénom;Nom;Téléphone;Email;Total dépensé (EUR);RDV;Devis;Source\n" +
        rows
          .map(
            (c) =>
      ...
      csv =
        "Numéro;Titre;Total (EUR);Statut;Date;Valide jusqu'au\n" +
        rows
          .map(
            (q) =>
      ...
      csv =
        "Titre;Date;Début;Fin;Statut\n" +
        rows.map((a) => `${a.title};${a.date};${a.startTime};${a.endTime};${a.status}`).join("\n");
