...
const PLANS = [
  ...
  },
  {
    ...
    features: [
      "Tout du Pro",
      "Assistant IA 24/7 (chatbot public)",
      ...
      "Chatbot IA sur vitrine",
      "Demande d'avis automatique",
      "Domaine personnalisé",
      "7 templates exclusifs",
    ],
  },
];
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    if (checkout === "success") {
      setCheckoutMsg(
        "🎉 Paiement réussi ! Votre abonnement sera actif dans quelques secondes (rechargez la page)."
      );
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900">
          ...
          <div className="space-y-4">
            ...
            <div className="border-t border-slate-100 pt-4 dark:border-slate-800 flex flex-wrap gap-4 text-sm">
              <a
                href="/cgu"
                target="_blank"
                className="flex items-center gap-1 text-slate-500 hover:text-slate-900 dark:hover:text-slate-100"
              >
                <FileText className="h-3.5 w-3.5" /> Conditions générales
              </a>
              <a
                href="/faq"
                target="_blank"
                className="flex items-center gap-1 text-slate-500 hover:text-slate-900 dark:hover:text-slate-100"
              >
                <FileText className="h-3.5 w-3.5" /> FAQ Vitrix
              </a>
              <a
                ...
                className="flex items-center gap-1 text-slate-500 hover:text-slate-900 dark:hover:text-slate-100"
              >
                <FileText className="h-3.5 w-3.5" /> Confidentialité
              </a>
              <a
                ...
                className="flex items-center gap-1 text-slate-500 hover:text-slate-900 dark:hover:text-slate-100"
              >
                <FileText className="h-3.5 w-3.5" /> Mentions légales
              </a>
            </div>
          </div>
        )}
        ...
        {tab === "langue" && (
          <div className="space-y-4">
            ...
            <p className="text-xs text-slate-500">
              Les boutons et titres de votre vitrine publique s&apos;afficheront dans cette langue
              pour vos clients.
            </p>
            <Button size="sm" onClick={saveLanguage} variant={saved ? "success" : "primary"}>
              {saved ? (
                <>
                  <Check className="mr-1 h-4 w-4" /> Enregistré — visible sur votre vitrine
                </>
              ) : (
                "Enregistrer"
              )}
        ...
        {tab === "abonnement" && (
          <div className="space-y-4">
            {checkoutMsg && (
              <div
                className={`rounded-xl p-4 text-sm ${checkoutMsg.startsWith("🎉") ? "bg-emerald-50 text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-400" : "bg-amber-50 text-amber-700 dark:bg-amber-900/20 dark:text-amber-400"}`}
              >
                {checkoutMsg}
              </div>
            )}
            ...
            {PLANS.map((plan) => {
              ...
              return (
                <div
                  key={plan.id}
                  className={`rounded-xl border p-5 ${isCurrent ? "border-slate-900 dark:border-white" : "border-slate-200 dark:border-slate-800"}`}
                >
                    ...
                    <div>
                      ...
                      {billing === "yearly" && plan.monthly > 0 && (
                        <p className="text-xs text-emerald-600">
                          soit {(plan.yearly / 12).toFixed(2)}€/mois — économisez{" "}
                          {plan.monthly * 12 - plan.yearly}€/an
                        </p>
                      )}
                    </div>
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <p className="text-sm font-medium text-slate-900 dark:text-slate-100">
                      Annuler mon abonnement {currentPlan === "premium" ? "Premium" : "Pro"}
                    </p>
                    <p className="text-xs text-slate-500">
                      Vous conserverez vos avantages jusqu'à la fin de la période payée, puis
                      passerez au plan Gratuit.
                    </p>
                  </div>
                  <Button
                    variant="outline"
                    size="sm"
                    className="shrink-0 text-red-600 border-red-200 hover:bg-red-50 dark:text-red-400 dark:border-red-900 dark:hover:bg-red-900/20"
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Utilisez votre propre nom de domaine (ex: www.mon-entreprise.fr) pour votre vitrine
                Vitrix.
              </p>
            </div>
            <div className="rounded-xl border border-slate-200 p-4 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50">
              ...
              <p className="text-xs text-slate-500 mb-4">
                Pour utiliser votre domaine, vous devez configurer un enregistrement CNAME chez
                votre registrar :
              </p>
            ...
            <div>
              ...
              <p className="mt-2 text-xs text-slate-500">
                Une fois configuré, contactez le support Vitrix pour activer le SSL et lier le
                domaine.
              </p>
            </div>
            <Button size="sm" disabled>
              Demander l&apos;activation (Support)
            </Button>
          </div>
        )}
        ...
        {tab === "domaine" && plan !== "premium" && (
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="mt-2 text-sm text-slate-500 max-w-md">
              Cette fonctionnalité est réservée au plan Premium. Elle vous permet d'utiliser votre
              propre nom de domaine (ex: www.mon-entreprise.fr) pour votre vitrine.
            </p>
            <Button className="mt-6" onClick={() => setTab("abonnement")}>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">
                    Téléchargez toutes vos données personnelles (compte, vitrine, clients, RDV,
                    devis, blog…) dans un fichier JSON structuré. Conforme à l&apos;article 20 du
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-red-600 dark:text-red-400">
                    Votre vitrine devient invisible immédiatement. Toutes vos données (clients,
                    devis, RDV, blog) sont conservées pendant
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-slate-600 dark:text-slate-400">
            ...
            {currentPlan === "premium"
              ? "Premium (IA, fidélité, marque blanche, templates exclusifs...)"
              : "Pro (réservations, devis, paiements, CRM...)"}{" "}
            à la fin de votre période payée. Votre vitrine restera en ligne sur le plan Gratuit.
          </p>
