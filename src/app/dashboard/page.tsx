...
export default function UnifiedDashboard() {
  ...
  const tabs = [
    ...
    {
      id: "rdv" as Tab,
      label: `${td("appointments")} (${data?.appointments?.length || 0})`,
      icon: CalendarDays,
    },
    ...
    ...(plan !== "free"
      ? [
          {
            id: "devis" as Tab,
            label: `${td("quotes")} (${data?.quotes?.length || 0})`,
            icon: FileText,
          },
        ]
      : []),
    {
      id: "clients" as Tab,
      label: `${td("clientsTab")} (${data?.clients?.length || 0})`,
      icon: Users,
    },
    {
      id: "paiements" as Tab,
      label: `${td("paymentsTab")} (${data?.payments?.length || 0})`,
      icon: CreditCard,
    },
  ];
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              <Button
                variant="outline"
                size="sm"
                className="border-white/20 text-white hover:bg-white/10 dark:border-slate-300 dark:text-slate-900"
                onClick={copyLink}
              >
                {copied ? (
              ...
              <Button
                ...
                className="border-white/20 text-white hover:bg-white/10 dark:border-slate-300 dark:text-slate-900"
                onClick={() =>
                  data.business?.slug && window.open(`/${data.business.slug}`, "_blank")
                }
              >
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-5">
            {[
              ...
              >
                <div
                  className={`mb-3 flex h-10 w-10 items-center justify-center rounded-xl ${s.color} text-white`}
                >
                    ...
                    {(data?.visits?.bySource || []).slice(0, 5).map((s) => {
                      ...
                      return (
                        <div key={s.source} className="flex items-center gap-2 text-sm">
                          <span className="w-24 truncate capitalize text-slate-600 dark:text-slate-400">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    {data?.reviews?.length ?? 0} avis clients
                  </p>
                </div>
              </div>
            </div>
          ...
          {stats.appointmentsCount === 0 && stats.quotesCount === 0 && stats.clientsCount === 0 && (
            <div className="rounded-2xl border-2 border-dashed border-slate-200 p-10 text-center dark:border-slate-800">
              ...
              <p className="mx-auto mt-2 max-w-md text-sm text-slate-500">
                Personnalisez votre vitrine, partagez votre lien et vos premiers rendez-vous, devis
                et clients apparaîtront ici.
              </p>
              <div className="mt-4 flex justify-center gap-2">
                <Link href="/dashboard/vitrine">
                  <Button size="sm">
                    <Palette className="mr-2 h-4 w-4" /> Personnaliser ma vitrine
                  </Button>
                </Link>
                <Link href="/dashboard/qr-code">
                  <Button variant="outline" size="sm">
                    <QrCode className="mr-2 h-4 w-4" /> Mon QR Code
                  </Button>
                </Link>
              </div>
            </div>
