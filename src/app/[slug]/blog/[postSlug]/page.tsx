  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950">
      <div className="mx-auto max-w-3xl px-4 py-12">
        ...
        {post.excerpt && (
          <p className="mt-4 text-lg text-slate-600 dark:text-slate-400 italic border-l-4 border-blue-500 pl-4">
            {post.excerpt}
          </p>
        )}
        ...
        <div className="mt-12 rounded-2xl bg-slate-50 p-8 dark:bg-slate-900 text-center">
          ...
          <p className="mt-2 text-sm text-slate-600 dark:text-slate-400">
            Contactez {biz.name} pour un devis gratuit.
          </p>
          <div className="mt-4 flex justify-center gap-3">
            <Link
              href={`/${biz.slug}`}
              className="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-6 py-3 text-sm font-semibold text-white hover:bg-slate-800 dark:bg-white dark:text-slate-900 dark:hover:bg-slate-100"
            >
              Retour à la page
            </Link>
            {biz.phone && (
              <a
                href={`tel:${biz.phone}`}
                className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-6 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-100 dark:border-slate-800 dark:text-slate-300"
              >
                📞 Appeler
              </a>
            )}
