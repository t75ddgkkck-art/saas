...
export default async function OgImage({ params }: Props) {
  ...
  try {
    ...
    if (business) {
      const rvws = await db
        ...
        .from(reviews)
        .where(eq(reviews.businessId, (await db.select({ id: businesses.id }).from(businesses).where(eq(businesses.slug, slug)).limit(1))[0]!.id));
  ...
  const primary = (business?.primaryColor && /^#[0-9a-fA-F]{6}$/.test(business.primaryColor))
    ...
    (
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          {city && (
            <div style={{ fontSize: 40, fontWeight: 500, color: "#cbd5e1" }}>📍 {city}</div>
          )}
          {desc && (
