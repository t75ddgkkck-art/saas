...
export function PublicPage({ business, hours, reviews, faqs, gallery, socials, slots, ownerPlan = "free", initialServices = [] }: PublicPageProps) {
  ...
  const coverStyle = business.coverImage 
    ? { backgroundImage: `url(${business.coverImage})`, backgroundSize: 'cover', backgroundPosition: 'center' }
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      fetch("/api/qr-code", {
        ...
        .then(r => r.json())
        .then(d => { if (d.qrCode) setQrCode(d.qrCode); })
        .catch(() => {});
      ...
      fetch("/api/quote-form-fields?business=" + business.slug)
        .then(r => r.json())
        .then(d => { if (d.fields) setQuoteFields(d.fields); })
        .catch(() => {});
  ...
  const [bookingForm, setBookingForm] = useState({ firstName: "", lastName: "", phone: "", email: "", notes: "" });
    ...
    <div className={`min-h-screen ${tpl.style.pageBg}`} style={{ fontFamily: tpl.style.fontFamily }}>
      ...
      >
        ...
        <div className="text-center">
          <h1 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-slate-100">{business.name}</h1>
          <div className="mt-3 flex flex-wrap items-center justify-center gap-3 text-sm">
            <Badge className="capitalize">{category}</Badge>
            {avgRating > 0 && (
              <div className="flex items-center gap-1.5">
                <Star className="h-4 w-4 fill-amber-400 text-amber-400" />
                <span className="font-semibold text-slate-900 dark:text-slate-100">{avgRating.toFixed(1)}</span>
                <span className="text-slate-500 dark:text-slate-400">({reviews.length} avis)</span>
              </div>
            )}
          </div>
          {business.description && (
            <p className="mx-auto mt-4 max-w-lg text-sm leading-relaxed text-slate-600 dark:text-slate-400">{business.description}</p>
          )}
        </div>
        ...
        <div className="mt-6 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            {business.phone && (
              <a href={`tel:${business.phone}`} className="flex items-center justify-center gap-2 rounded-xl bg-slate-900 py-3.5 text-sm font-semibold text-white transition-colors hover:bg-slate-800 dark:bg-white dark:text-slate-900 dark:hover:bg-slate-100">
        ...
        <div className="mt-4 space-y-3">
          ...
          <div className={`grid gap-3 ${canQuote ? "grid-cols-2" : "grid-cols-1"}`}>
            {canQuote && (
            <Button variant="outline" className="h-12" leftIcon={<FileText className="h-4 w-4" />} onClick={() => setShowQuote(true)}>
              {t(lang, "quote")}
            </Button>
            )}
                      ...
                      <div key={j} className="flex gap-4 group">
                        ...
                        <div className="flex-1">
                          <div className="flex justify-between items-baseline border-b border-dashed border-slate-200 dark:border-slate-700 pb-1">
                            ...
                            <span className="font-bold text-amber-600 dark:text-amber-500 ml-2">{item.price}</span>
                          </div>
                          {item.description && (
          ...
          <div className="mt-8 rounded-2xl border border-slate-200 bg-white p-5 sm:p-6 dark:border-slate-800 dark:bg-slate-900 shadow-sm">
            ...
            <div className="space-y-3">
              {servicesList.map((s, i) => (
                <div key={i} className="group flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl bg-slate-50 border border-slate-100 hover:border-blue-200 hover:shadow-md transition-all dark:bg-slate-800/50 dark:border-slate-800 dark:hover:border-blue-900/50">
                  <div className="flex-1 min-w-0 mb-2 sm:mb-0">
                    <div className="flex items-center gap-2">
                      <p className="font-bold text-slate-900 dark:text-slate-100 text-[15px]">{s.name}</p>
                      {i === 0 && <span className="text-[10px] bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded font-medium dark:bg-amber-900/30 dark:text-amber-400">Populaire</span>}
                    </div>
                    {s.description && (
          ...
          <div className="mt-8 rounded-2xl border border-blue-200 bg-blue-50/50 p-6 dark:border-blue-900/50 dark:bg-blue-900/10">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">Réglez vos factures ou acomptes directement par carte bancaire.</p>
              </div>
            ...
            <div className="space-y-1 flex items-center gap-2 mb-5">
              <div className="flex">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star key={i} className={`h-5 w-5 ${i < Math.round(avgRating) ? "fill-amber-400 text-amber-400" : "text-slate-200 dark:text-slate-700"}`} />
                ))}
              </div>
              <span className="text-sm font-medium text-slate-900 dark:text-slate-100">{avgRating.toFixed(1)}/5</span>
              <span className="text-sm text-slate-500 dark:text-slate-400">({reviews.length} avis)</span>
            </div>
          ) : (
        ...
        <div className="mt-8 flex justify-center">
          <a href={`/${business.slug}/blog`} className="flex items-center gap-2 rounded-xl border border-slate-200 px-6 py-3 text-sm font-medium text-slate-700 transition-colors hover:border-slate-900 hover:bg-slate-50 dark:border-slate-800 dark:text-slate-300 dark:hover:border-white">
            📝 {t(lang, "blog")}
          </a>
        </div>
      ...
      {showBooking && (
        <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center">
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm" onClick={() => { setShowBooking(false); setBookingStep(0); setSelectedSlot(null); setBookingSuccess(false); }} />
          <div className="relative z-10 w-full max-w-lg rounded-t-2xl bg-white p-6 dark:bg-slate-900 sm:rounded-2xl max-h-[90vh] overflow-y-auto">
            <div className="mb-4 flex items-center justify-between">
              <div>
                <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Prendre rendez-vous</h3>
            ...
            {bookingSuccess ? (
              <div className="py-8 text-center">
                ...
                <h4 className="text-lg font-bold text-slate-900 dark:text-slate-100">Rendez-vous confirmé !</h4>
                <p className="mt-2 text-sm text-slate-600 dark:text-slate-400">
                  {bookingForm.firstName}, vous êtes attendu(e) le <strong>{selectedSlot?.split("_")[0]}</strong> à <strong>{selectedSlot?.split("_")[1]}</strong>.
                </p>
                <p className="mt-1 text-xs text-slate-500">Un rappel vous sera envoyé avant le rendez-vous.</p>
                <Button className="mt-6" onClick={() => { setShowBooking(false); setBookingStep(0); setSelectedSlot(null); setBookingSuccess(false); }}>Fermer</Button>
              </div>
            ) : bookingStep === 0 ? (
              <>
                {slots.length > 0 ? (
                  <div className="max-h-72 space-y-4 overflow-y-auto">
                    {Object.entries(groupedSlots).slice(0, 7).map(([date, daySlots]) => (
                      <div key={date}>
                        <p className="mb-2 text-sm font-semibold text-slate-700 dark:text-slate-300">{date}</p>
                  ...
                  <div className="rounded-xl bg-slate-50 p-6 text-center dark:bg-slate-800">
                    ...
                    <p className="text-sm text-slate-500 dark:text-slate-400">Aucun créneau disponible.</p>
                    {business.phone && (
                      <a href={`tel:${business.phone}`} className="mt-3 inline-flex items-center gap-2 text-sm font-medium text-blue-600 hover:underline">
                        <Phone className="h-4 w-4" /> Appelez-nous
                      </a>
                    )}
                  </div>
                )}
                <Button className="mt-6 w-full" disabled={!selectedSlot} onClick={() => selectedSlot && setBookingStep(1)}>
                  Continuer
                </Button>
              </>
            ) : bookingStep === 1 ? (
              <div className="space-y-4">
                <div className="rounded-xl bg-slate-50 p-4 dark:bg-slate-800">
                  <p className="text-sm font-medium text-slate-900 dark:text-slate-100">Créneau sélectionné</p>
                  <p className="text-sm text-slate-600 dark:text-slate-400">{selectedSlot?.split("_")[0]} à {selectedSlot?.split("_")[1]}</p>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <Input label="Prénom" value={bookingForm.firstName} onChange={(e) => setBookingForm({ ...bookingForm, firstName: e.target.value })} placeholder="Jean" />
                  <Input label="Nom" value={bookingForm.lastName} onChange={(e) => setBookingForm({ ...bookingForm, lastName: e.target.value })} placeholder="Dupont" />
                </div>
                <Input label="Téléphone" value={bookingForm.phone} onChange={(e) => setBookingForm({ ...bookingForm, phone: e.target.value })} placeholder="+336 12 34 56 78" />
                <Input label="Email (pour recevoir votre confirmation)" type="email" value={bookingForm.email} onChange={(e) => setBookingForm({ ...bookingForm, email: e.target.value })} placeholder="jean@email.fr" required />
                <Input label="Motif (optionnel)" value={bookingForm.notes} onChange={(e) => setBookingForm({ ...bookingForm, notes: e.target.value })} placeholder="Ex: Réparation fuite" />
                <div className="flex gap-3">
                  <Button variant="outline" className="flex-1" onClick={() => setBookingStep(0)}>Retour</Button>
                  <Button className="flex-1" loading={bookingLoading} onClick={async () => {
                    if (!bookingForm.firstName || !bookingForm.phone || !bookingForm.email) return;
                    ...
                    try {
                      ...
                      const res = await fetch("/api/book-appointment", {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({ businessId: business.id, date, startTime, endTime: selectedSlot!.split("_")[1].replace(/(\d+):(\d+)/, (_, h) => `${String(Number(h) + 1).padStart(2, "0")}:00`), ...bookingForm }),
                      });
                      if (res.ok) {
      ...
      {showReviewForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setShowReviewForm(false)} />
          <div className="relative z-10 w-full max-w-md rounded-2xl bg-white p-6 dark:bg-slate-900 shadow-2xl">
            ...
            <div className="space-y-4">
              <div>
                <label className="mb-2 block text-sm font-medium text-slate-700 dark:text-slate-300">Votre note</label>
                <div className="flex gap-1">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <button key={star} onClick={() => setReviewRating(star)} className="text-2xl">
                      <Star className={`h-8 w-8 ${star <= reviewRating ? "fill-amber-400 text-amber-400" : "text-slate-300 dark:text-slate-600"}`} />
                    </button>
                  ))}
                </div>
              </div>
              <Input label="Votre nom" value={reviewName} onChange={e => setReviewName(e.target.value)} placeholder="Jean D." />
              <Textarea label="Votre avis" value={reviewText} onChange={e => setReviewText(e.target.value)} placeholder="Partagez votre expérience..." rows={4} />
              <Input
                label="Votre email"
                type="email"
                value={reviewEmail}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-md rounded-2xl bg-white p-6 dark:bg-slate-900 shadow-2xl">
            ...
            <p className="mb-4 text-sm text-slate-500 dark:text-slate-400">
              Vous effectuez un paiement vers <strong>{business.name}</strong>. Vous serez redirigé vers Stripe pour finaliser la transaction.
            </p>
            <div className="space-y-4">
              <div>
              ...
              <div>
                <label className="mb-1 block text-sm font-medium text-slate-700 dark:text-slate-300">Motif (optionnel)</label>
                <Input placeholder="Ex: Acompte, Facture #123..." value={payDesc} onChange={e => setPayDesc(e.target.value)} />
              </div>
              <Button className="w-full" size="lg" onClick={handlePayment} loading={payLoading} disabled={!payAmount || parseFloat(payAmount) <= 0}>
                <CreditCard className="mr-2 h-4 w-4" /> Payer {payAmount ? `${parseFloat(payAmount).toFixed(2)} €` : ""}
              </Button>
            </div>
          </div>
        </div>
